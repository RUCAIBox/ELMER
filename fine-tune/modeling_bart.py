...
class BartModel(BartPretrainedModel):
    ...
    def forward(
        ...
        decoder_outputs, all_intermediate_logits = self.decoder(
        ...
        if not return_dict:
            return decoder_outputs + encoder_outputs, exited_hidden_states
...
class BartForConditionalGeneration(BartPretrainedModel):
    ...
    def forward(
        ...
        outputs, all_intermediate_logits = self.model(
            input_ids,
            attention_mask=attention_mask,
        ...
        masked_lm_loss = None
        intermediate_lm_losses = []
        if labels is not None:
            loss_fct = CrossEntropyLoss(ignore_index=self.model.config.pad_token_id)

            # using all intermediate logits and final logits to compute losses
            for logits in all_intermediate_logits:
                inter_logits = logits + self.final_logits_bias
                intermediate_lm_losses.append(loss_fct(inter_logits.view(-1, self.config.vocab_size), labels.view(-1)))

            masked_lm_loss = loss_fct(lm_logits.view(-1, self.config.vocab_size), labels.view(-1))

        if not return_dict:
            output = (lm_logits,) + outputs[1:]
            return ((masked_lm_loss,) + output) if masked_lm_loss is not None else output, intermediate_lm_losses
