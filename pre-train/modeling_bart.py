...
def _make_backward_causal_mask(input_ids_shape: torch.Size, dtype: torch.dtype, past_key_values_length: int = 0):
    """
    Make causal mask used for bi-directional self-attention.
    """
    bsz, tgt_len = input_ids_shape
    mask = torch.full((tgt_len, tgt_len), float("-inf"))
    mask_cond = torch.arange(mask.size(-1))
    mask.masked_fill_(mask_cond + 1 > mask_cond.view(mask.size(-1), 1), 0)
    mask = mask.to(dtype)

    if past_key_values_length > 0:
        mask = torch.cat([torch.full((tgt_len, past_key_values_length), float("-inf")), mask], dim=-1)
    return mask[None, None, :, :].expand(bsz, 1, tgt_len, tgt_len + past_key_values_length)
...
def _expand_mask(mask: torch.Tensor, dtype: torch.dtype, tgt_len: Optional[int] = None):
    ...
    inverted_mask = 1.0 - expanded_mask  # mask: 1 for not masked 0 for masked, so it needs to be inverted
    ...
    def forward(
        ...
        for idx, encoder_layer in enumerate(self.layers):
            if output_hidden_states:
                encoder_states = encoder_states + (hidden_states,)  # add input embedding layer
...
class BartDecoder(BartPretrainedModel):
    ...
    def _prepare_decoder_backward_attention_mask(self, attention_mask, input_shape, inputs_embeds, past_key_values_length):
        # create causal mask
        # [bsz, seq_len] -> [bsz, 1, tgt_seq_len, src_seq_len]
        combined_attention_mask = None
        if input_shape[-1] > 1:
            combined_attention_mask = _make_backward_causal_mask(
                input_shape, inputs_embeds.dtype, past_key_values_length=past_key_values_length
            ).to(self.device)
    ...
    def forward(
        ...
        # ELMER also adopts the bidirectional mask in decoder
        if attention_mask is not None:
            # [bsz, seq_len] -> [bsz, 1, tgt_seq_len, tgt_seq_len]
            attention_mask = _expand_mask(attention_mask, inputs_embeds.dtype)
        ...
        # collect exit hidden states for computing losses
        # batch_size * seq_len * hidden_size
        exit_hidden_states = torch.zeros(hidden_states.size()).to(hidden_states.device)
        has_all_exited = torch.zeros(input_shape).unsqueeze(-1).bool().to(hidden_states.device)
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            hidden_states = layer_outputs[0]  # [bsz, seq_len, embed_size]
            ...
            if exit_layers is not None:
                ...
                exited_signal = torch.eq(exit_layers, idx).unsqueeze(-1)
                ...
                copy_signal = torch.le(exit_layers, idx).unsqueeze(-1)
...
class BartModel(BartPretrainedModel):
    ...
    def forward(
        ...
        if not return_dict:
            return decoder_outputs + encoder_outputs, exit_hidden_states
...
class BartForConditionalGeneration(BartPretrainedModel):
    ...
    def forward(
        ...
        exit_layers (`torch.LongTensor` of shape `(batch_size, sequence_length)`):
            Sampled exit layers for each token. Indices should either be in `[0, ..., config.decoder_layers-1]`.
            If the exit layer is set to j, the model will exit and predict tokens at the l-th decoder layer.
        ...
        outputs, exit_hidden_states = self.model(
            input_ids,
            attention_mask=attention_mask,
        ...
        lm_logits = self.lm_head(exit_hidden_states) + self.final_logits_bias

        masked_lm_loss = None
        if labels is not None:
            loss_fct = CrossEntropyLoss(ignore_index=self.model.config.pad_token_id)
